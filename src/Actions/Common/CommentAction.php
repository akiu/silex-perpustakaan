...
class CommentAction extends BaseAction
{
    public function handle($comment, $id)
    {
        ...
        $userName = $conn->fetchAssoc("SELECT username FROM user WHERE id = ?", [$userId]);

        $imagePath = $conn->fetchAssoc("SELECT profilePicturePath FROM userProfile WHERE userId = ?", [$userId]);
